...
from wochat.config import ROOT, settings  # noqa: E402
from wochat.store.models import Alert, AnalysisResult, CrawlTask  # noqa: E402
...
def _run_cli(args: list[str], timeout: int = 1800) -> tuple[int, str]:
    ...
    try:
        proc = subprocess.run(
            [sys.executable, "-m", "wochat.cli", *args],
            cwd=str(ROOT),
            env=env,
            capture_output=True,
            text=True,
...
def tab_crawl() -> None:
    ...
    st.warning(
        ...
        "在远程/无头环境里跑不通 —— 那种情况请在终端执行 `python -m wochat.cli crawl ...`。"
        "只想验证链路就用 `mock` 后端。"
    )
    ...
    st.divider()
    st.markdown("#### 采集任务队列")
    st.caption("调度器（`python -m wochat.scheduler.jobs`）会周期性消费 status 为 pending/running 的任务。")
    rows = _query(
        lambda r: [
            {
                "task_id": t.task_id,
...
def tab_clean() -> None:
    ...
    from wochat.crawler.base import anonymize_id
    from wochat.pipeline import dedup as dedup_mod
    from wochat.pipeline.llm_clean import LLMCleaner
    from wochat.pipeline.rules import clean, is_spam, tokenize
...
def tab_store() -> None:
    ...
    stats = _query(lambda r: r.stats())
    st.json(stats)
    st.caption(f"数据库：`{settings.store.url}`　|　迁移 PostgreSQL 只需改 `WOCHAT_DB_URL`。")
