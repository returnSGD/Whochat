...
典型用法：
    ...
    # 3. 看板
    python -m wochat.cli dashboard

    # 4. 真实采集（需要 MediaCrawler 环境）
    python -m wochat.cli crawl --platform xhs --keyword "某品牌"

    # 5. 导入已有的 MediaCrawler 输出
    python -m wochat.cli import vendor/MediaCrawler/data/xhs/jsonl/search_comments_2026-09-11.jsonl --platform xhs

    # 6. 分析 / 主题 / 词云 / 预警
    python -m wochat.cli analyze
    python -m wochat.cli topics
    python -m wochat.cli wordcloud
    python -m wochat.cli alert
"""
...
def _repo():
    from wochat.store.repository import Repository, init_db
...
def cmd_init(args) -> int:
    _banner("初始化")
    from wochat.alert.rules_engine import seed_default_rules
    ...
    from wochat.analysis.sentiment import TransformerSentiment, get_analyzer
    ...
    from wochat.pipeline.llm_clean import LLMCleaner
    ...
    from wochat.analysis.wordcloud_gen import find_cjk_font
...
def cmd_demo(args) -> int:
    ...
    from wochat.pipeline.runner import PipelineStats
    ...
    # 词云
    print()
    from wochat.analysis import wordcloud_gen
    ...
    _banner("完成")
    print(stats.report())
    print(f"\n下一步: python -m wochat.cli dashboard")
...
def cmd_import(args) -> int:
    ...
    from wochat.crawler.base import CrawlTask, register
    from wochat.crawler.manual_source import ManualImportSource, register_manual
    from wochat.pipeline.runner import Pipeline
    ...
    _banner("导入完成")
    print(stats.report())
    print("\n下一步: python -m wochat.cli analyze")
    return 0
...
def cmd_wordcloud(args) -> int:
    ...
    if not texts:
        print(f"没有找到版本 {version} 的分析结果。先跑: python -m wochat.cli analyze")
        return 1
...
def cmd_status(args) -> int:
    ...
    print(f"看板:   python -m wochat.cli dashboard")
    return 0
...
def cmd_evaluate(args) -> int:
    ...
    from wochat.analysis.sentiment import evaluate, get_analyzer
