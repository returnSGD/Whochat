...
> **最新一轮见 §十七 —— LLM 分析接入并在操作端可直接配置：
> 只需 base_url + api_key，可在 /console 的 L2 页面里填写。**
> 历史轮次：§九（第二轮加固，修 27 个缺陷 + 补 130 测试）、
> §十~十三（第三轮加固 + 传播可视化 + 双入口看板 + 全项目改名）、
> §十四~十五（端口与路由两个"HTTP 200 骗人"的坑）、
> §十六（LLM 打标 / 字段映射 / 主题命名，只填两个值即可用）。
...
| L2 清洗 | LLM 打标（广告/主体/关键词） | ✅ **已接入流水线并端到端验证**（§十六）。OpenAI 兼容接口，只填 URL+key |
| L2 清洗 | LLM 字段映射（陌生字段名） | ✅ 已验证（§十六）：学一次 + 落盘缓存，`import --llm-map` |
...
| L3 分析 | LLM 主题命名 | ✅ 已验证（§十六）：碎片关键词 → 一句人话 |
...
代码量：**约 6800 行**（`src/` 下 33 个 Python 文件）；**测试 308 个用例**
（第二轮 130 → 第三轮 192 → §十六 278 → §十七 308）
...
| ~~**本地 LLM 清洗**~~ | ✅ **已由 §十六 取代**：不再需要 Ollama，改成任何 OpenAI 兼容接口，只填 `WHOCHAT_LLM_BASE_URL` + `WHOCHAT_LLM_API_KEY` |
...
## 十六、LLM 分析接入（2026-09-12）

按 §15 之前讨论的优先级做了 **1（广告/主体/关键词打标）→ 5（字段映射）→ 3（主题命名）**。
核心诉求是「分析者只需要提供请求地址和 api key」—— 这条决定了整个选型。
...
为什么走这套协议：它已是事实标准，DeepSeek / 通义千问 / Moonshot / 智谱 /
OpenAI / 以及本地 Ollama 的 `/v1` 端点全都实现了。换供应商只改 base_url，
调用方一行不动 —— 与 `CrawlerSource` 协议同一个思路（ADR#1 的教训）。

**没有引入任何 SDK**（`openai` / `ollama` / `instructor` 都不需要）。
只需要一个 POST，而 `requests` 本来就是核心依赖 —— 这才保住了
「配两个环境变量就能用」这个承诺。`pyproject.toml` 的 `llm` extra
因此从 `ollama+instructor+json-repair` 缩到只剩 `json-repair`。

### 16.2 上手成本被压到最低的三处设计

| 设计 | 解决什么 |
|---|---|
| **模型名可留空** | 自动调 `GET /models` 挑一个对话模型，排除 embed/rerank/whisper 等。挑不出来**明确报错**而不是瞎猜（猜错的表现是"每条都失败但静默"） |
| **端点自动探测** | 只写 `https://api.deepseek.com` 也行。在 `{base}/chat/completions` 与 `{base}/v1/chat/completions` 间探测一次并缓存 |
| **开关自动推断** | `WHOCHAT_LLM_ENABLED` 没设时，只要 base_url 与 api_key 都填了就算开启 —— 不该再让人翻文档找那个额外的开关 |

### 16.3 任务一：LLM 打标（`pipeline/llm_clean.py` + `llm_client.py`）

规则法在这三个任务上都有硬天花板：
- **广告识别**：`rules.py::is_spam` 是关键词正则，第三轮就出过误杀 ——
  「商家刷单太明显了，太失望了」是本该被分析的**负面舆情**，却因为含"刷单"
  被当广告永久排除出情感统计。这是**语义判断**，正则做不了。
- **主体识别**：`analysis_results.subject` 建表起就一直空着，规则无能为力。
- **关键词**：jieba+TF-IDF 抽的是高频词，不是"这条在说什么"。

**情感判定仍然永远走词典法**（ADR#5）。集成测试里专门钉了这条：假模型返回
`sentiment_hint=positive`，入库的仍是词典法判的 `negative`。

#### 批量与对齐：本模块最容易出事的地方

批量是省钱关键（1 次请求 vs K 次，1 万条约 1000 次）。但批量引入**漏项风险**，
所以定了两条铁律：

1. **返回列表必须与输入等长且顺序一致** —— 调用方按 `zip(comments, results)`
   消费，长度对不上就会把 A 的标签贴到 B 身上，且完全静默。
2. **漏项/失败一律放行，绝不丢数据** —— 那条评论继续被正常分析，只是不带
   LLM 标签。漏项条目被标记成 `error="模型漏项"`、`is_ad=None`，
   **绝不能**伪造成"模型判定它不是广告"（那是最坏的静默错误：看不出有东西丢了）。

### 16.4 找到并修掉的 4 个真实缺陷

| # | 位置 | 问题 | 影响 |
|---|---|---|---|
| 1 | `llm_clean.py` | 对齐用**逐条回退**：模型返回 i=1,2 而输入 2 条时，第 2 条越界回退到"位置 1"，把刚对齐好的覆盖掉 | 位置 0 永远空着被记成"漏项"，位置 1 只剩后一条。随返回顺序变化，极难排查 |
| 2 | `analysis/topics.py` | **同一个 bug 又写了一遍**（因为我复制了对齐逻辑） | 同上。抽成共用的 `align_by_index()` 后不再漂移 |
| 3 | `llm_client.py` | `align_by_index` 用 `break` 跳垃圾条目 | 中间夹一条垃圾就把**后面所有有效结果**一起丢掉 |
| 4 | `crawler/llm_map.py` | 缓存键只含字段名、**不含目标 schema** | 内容与评论的原始字段名常常是同一批，先学的内容映射会被评论那轮命中、按 COMMENT_SPEC 过滤成空 → **评论字段永远学不到且不报错** |

第 1、2 条是同一个错误犯两次 —— 根因是逻辑重复，所以抽成了共用函数。
第 4 条是写测试时才暴露的：单测里"同一批字段名、两套 spec"的场景直接把它照出来了。

### 16.5 任务二：字段映射（`crawler/llm_map.py`）

别名表是**手写**的，天花板第三轮撞过：MediaCrawler 真实字段 `creator_hash` /
`user_nickname` 没被收录，导致真实采集的作者 ID 恒为 NULL、KOL 识别完全失效。
LLM 能做这件事是因为"把 `creator_hash` 认成作者 ID"是语义判断，而且
**能泛化到没见过的平台**，别名表不能。

**关键设计：学一次，不是每条都问。** 字段名在同一平台内是稳定的，所以对
一组未知字段名问一次就够，结果落盘缓存（`data/llm_field_map.json`）后机械套用。
逐条调模型不可行 —— 一个采集任务几万条能烧掉几百块。

防护（三条都写了测试）：
- 只接受指向目标 schema 内字段的映射，模型编的字段名一律丢掉
- 一个目标字段只认一次
- **学到的映射追加在别名表末尾** —— 手写别名表是人工核对过的，优先级必须更高

用法：`python -m Whochat.cli import weird.jsonl --platform newplat --llm-map`

### 16.6 任务三：主题命名（`analysis/topics.py::name_topics`）

聚类出来的是 c-TF-IDF 关键词碎片，`Topic.label` 原本是 `" / ".join(words[:3])`，
即「发热 / 续航 / 掉电」这种线索，不是人能直接用的主题名。

只替换 `label`，**关键词原样保留** —— 所以重命名不丢信息。命名失败时
**保留原标签**（不清空、不置 None）：主题名难看总好过看板上一片空白。

### 16.7 幂等性的处理方式

`demo` 的「跑两次数字不变」是这个项目的回归基线（README 明确写了）。
LLM 是概率性的，直接接进主链路会废掉这条基线。做法是**版本隔离**：

```
未开 LLM  →  lexicon-v1        （完全不变）
开启 LLM  →  lexicon-v1-llm    （并存，不覆盖）
```

同一批数据两种结果**并存**，事后能对比"用没用模型"的差异 —— 这也正是
`analysis_version` 字段当初的设计意图。实测两种版本各 1928 行并存，
旧版本的 `subject` 仍是 NULL，没被改写。

### 16.8 验证（哪些是真的，哪些还没验证）

**已实测**：

- `pytest -q` **278 passed / 2 xfailed**（192 → 278，新增 86 条）
- `demo` 幂等复检：内容 60 / 评论 1928 / 分析 1164，版本仍为 `lexicon-v1`，
  第二次 `分析 0 条` —— **与基线完全一致**
- `cli init` 自检：未配置时提示怎么配；配置后显示
  `就绪 · 模型 fake-chat（自动选择（候选 2 个））`
- **端到端**：起了一个假的 OpenAI 兼容服务（`/v1/models` +
  `/v1/chat/completions`），**只设 base_url 与 api_key**，跑完整 `cli analyze`：
  模型自动选中 `fake-chat`（跳过了 `text-embedding-3`）、1164 条打标、
  两个版本各 1928 行并存、`subject`/`keywords` 落库、
  且**情感仍是词典法判的 `negative`（模型给的是 neutral）—— ADR#5 得到遵守**

**测试用的假服务而不是 mock `requests`**：客户端最易出错的是 HTTP 层
（端点探测、状态码分支、退避重试、JSON Mode 降级）。把 `post` 换掉就等于
把要测的东西测掉了。桩服务让 `requests` 真的走 socket，
整个过程**不需要 API key、不需要外网**。

**⚠️ 没有验证的**：真实供应商的兼容性（DeepSeek / 通义 / Moonshot 各家的
`response_format`、字段名微差），以及 **LLM 打标的准确率** ——
测试钉的是**契约**（不漏项、不错位、不丢数据），不是"判得准不准"。
广告识别比正则好在哪、差在哪，需要业务标注集来评。

### 16.9 一个顺带发现

测试里用 `"a"` / `"b"` / `"c"` 这类单字母当样本时，**规则层会把它们判成垃圾
直接短路，请求根本到不了模型** —— 测试全绿但测的是别的东西。
已改用真实评论句，并在测试文件里写明这个坑。

---

## 十七、操作端可直接配置 LLM（2026-09-12）

§十六 把 LLM 做成了「填两个环境变量就能用」，但配置入口只在 `.env` 里。
用户反馈：**L2 页面上只显示一段"去 .env 里填这两个值"的说明，不友好。**

这个反馈是对的，而且不只是不友好 —— 操作端（`/console`）的定位就是
**「可写」**（改预警规则、触发分析都在这里），唯独 LLM 配置让人去手改文件，
和页面定位自相矛盾。

### 17.1 改动

L2 清洗页的「LLM 分析」区块改成三个输入框 + 两个按钮：

| 控件 | 说明 |
|---|---|
| 请求地址 base_url | 预填当前值 |
| API Key | **password 类型，永不回显**；留空 = 不修改 |
| 模型名 | 可留空（自动挑） |
| 保存 / 保存并测试连接 | 写 `.env` → 刷新当前进程 → 可选立即发一次真实请求验证 |

写进 `.env` 而不是另起一套存储：它是本项目**唯一**的配置来源
（`load_dotenv` 在模块导入时读它），且已在 `.gitignore` 里。

### 17.2 三个必须处理的细节

1. **保存后要刷新当前进程**。`Settings` 是**导入时的快照**，只写文件的话
   页面会显示"已保存"但紧接着的测试连接仍用旧值 —— 表现得像保存没生效。
   所以 `save_llm_settings()` 同时更新 `os.environ` 与内存里的 `settings.llm`。
2. **密钥永不回显**。输入框 `value` 恒为空、`type=password`，已保存时只显示
   `mask_secret()` 的掩码形态（`sk-a***wxyz`）。因此"空字符串"无法区分
   "没填"和"想清空"，所以约定 `api_key=None` 表示**不改动**。
3. **必须原地改 `.env` 的既有行**，不能追加 —— 否则同一个键出现两次，
   取决于解析顺序，是典型的"改了半天不知道哪个生效"。

### 17.3 顺带查出一个真 bug：空值把 LLM 开关关掉了

写 §16 时定的规则是「`WHOCHAT_LLM_ENABLED` 没设时，只要 base_url 与 api_key
都填了就算开启」，靠它兑现"只填两个值就能用"。

但 `_env_bool_or_none()` 当时写成：

```python
return None if os.getenv(name) is None else _env_bool(name)
```

**空字符串被当成了显式 False**。而 `.env.example` 里那一行恰恰就是留空的
`WHOCHAT_LLM_ENABLED=` —— 于是**每一个从 `.env.example` 复制配置的人，
即使把 URL 和 key 都填好，LLM 也永远是关的**，且界面上没有任何异常，
只是安静地不生效。

这正是 §16 想避免的那类失败，只是换了个地方。修法：空白（`None` / `""` /
纯空格 / `\t`）一律视为"没设"。三种状态实测：

```
WHOCHAT_LLM_ENABLED=''       → is_enabled = True   （自动）
WHOCHAT_LLM_ENABLED='   '    → is_enabled = True   （自动）
WHOCHAT_LLM_ENABLED='false'  → is_enabled = False  （显式关）
```

另外，页面上保存时会把 `WHOCHAT_LLM_ENABLED` 显式写成 `true`：在页面上填地址
和密钥这个动作本身就表示"我要用"。不写的话，若 `.env` 里原本是 `false`，
就会出现"界面里配好了、重启后又变回关的"最难排查的那种不一致。

> ⚠️ 排查过程中还踩到一个坑：**`LLMConfig` 的字段是裸 dataclass 默认值，
> 在导入时就固化了**。所以在同一个进程里改 `os.environ` 再构造 `LLMConfig()`
> 拿到的仍是旧值 —— 我第一版的验证就是这么做的，结论全错。这类三态判断
> 必须用**子进程**验（`WHOCHAT_LLM_ENABLED=x python -c ...`）才作数。
> （`WebConfig` 当初用 `default_factory` 就是绕开这一点；`LLMConfig` 尚未改，
> 因为 `settings` 单例本就只在导入时构造一次，不影响实际行为。）

### 17.4 验证

- `pytest -q` **308 passed / 2 xfailed**（278 → 308，新增 30 条）
- 新增的验收用例**真的在页面上填表并点保存**，断言 `.env` 被写出、
  且当前进程立即生效（`is_enabled` 变 True）—— 前面几条只证明"输入框存在"
- 密钥框 `proto.type == 1`（password），且已配置时 `value == ""`、
  placeholder 里只有掩码
- 地址为空时报错且**不写出**空配置
