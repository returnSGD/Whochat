...
┌──────────────────────────┐   ┌──────────────────────────────────┐
│ L5 预警分发               │   │ L6 可视化看板                     │
│ 聚合窗口·分级·冷却        │   │ FastAPI + 前端 · localhost:8501   │
...
你要的 `localhost:8501` 页面，严格讲是**可视化看板（Dashboard）**，不是数据中台。真正的中台是数据资产管理 + 服务层。
...
默认端口 **8501**（`WHOCHAT_DASHBOARD_PORT` 可改）；MediaCrawler 自带的 WebUI 占 8080，注意错开。

> ⚠️ **选端口时避开浏览器禁用端口**。Chrome / Edge / Firefox 都内置一份受限端口
> 清单（`6665~6669` 等原 IRC 端口段在内），浏览器会在建连前直接拒绝，报
> "无法访问此页面"；而 `curl` / `requests` **不检查**这份清单，服务端一切正常。
> 结果是：`init` / `pytest` / HTTP 200 全绿，唯独人打不开 —— 本方案初版随手定的
> `6666` 正是踩了这个坑（见 WORKLOG §十四）。
